...
<React.Fragment>
      ...
      {this.state.display && (
        ...
        <div className="languages__outer">
          <p className="languages__text">{this.state.frenchClicked ? "" : text[language].readingLanguage}</p>
          <p className="languages__text languages__text-warning">{this.state.frenchClicked ? "Je suis désolée mais le français n'est pas encore implementé." : text[language].changeDefault}</p>
            ...
            <div className="languages__flags--inner" lang="fr" onClick={this.frenchSelected}>
              ...
              <img className={`languages__flag notYet ${language === 'en' ? 'languages__flag--active' : ''}`} src="https://www.countryflags.io/fr/shiny/64.png" alt="" />
					...
					<p className="languages__text languages__text--checkbox">
						<input type="checkbox" className="languages__checkbox" onClick={this.handleShowLanguagePopupAgain} defaultChecked />{text[language].chekboxText}
					</p>
